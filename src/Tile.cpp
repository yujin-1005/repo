...
Tile::Tile(unsigned int T_R, unsigned int T_S, unsigned int T_C, unsigned int T_K, unsigned int T_G, unsigned int T_N, unsigned int T_X_, unsigned int T_Y_, bool folding) {
    this->T_R = T_R; //Number of flter rows mapped at a time
    this->T_S = T_S; //Number of filter columns mapped at a time
    this->T_C = T_C; //Number of filter and input channels per group mapped at a time
    this->T_K = T_K; //Number of filters and output channels per group mapped at a time
    this->T_G = T_G; //Number of groups mappd at a time
    this->T_N = T_N; //Number of inputs mapped at a time (Only 1 is supported so far)
    this->T_X_ = T_X_; //Number of input rows mapped a time
    this->T_Y_ = T_Y_; //Number of input columns mapped a time
          
    this->VN_Size = T_R*T_S*T_C; //yujin: Number of filter mapped at a time
    this->Num_VNs = T_K*T_G*T_N*T_X_*T_Y_; 
    this->folding = folding;
    if(this->folding) { //yujin: folding이 되면, 이전에 계산하는 값을 (중간)psum으로 받아 현재 연산과 더해주어 psum을 구해야함 -> 따라서 VN_size++
