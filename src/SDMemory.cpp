...
SDMemory::SDMemory(id_t id, std::string name, Config stonne_cfg, Connection* write_connection) : MemoryController(id, name) {
    ...
    // write_connection : Output of the lookup table connection and write port to the SDMemory
    ...
    this->ms_size_per_input_port = this->num_ms / this->n_read_ports;  //n_read_ports = dn_bw
...
void SDMemory::setTile(Tile* current_tile) {
    ...
    } //yujin: folding인 경우 현재 iteration을 check할 필요가 있음 
    ...
    //Number of output psums per each channel. Used t avoid sending packages of new k iterations if the previous have not been 
    //calculated yet;
...
void SDMemory::cycle() {
    ...
    // DataPackage : ART Package constructor (only one package for this type) -> DataPackage.h
    ...
    // current number of output pixels calculated/output_psums_per_channel
    unsigned index_G=current_G*this->current_tile->get_T_G(); // current_Group*Number of groups mappd at a time
    unsigned index_K=current_K*this->current_tile->get_T_K(); // current Number of filters and output channels*current_tile_filter
    unsigned int pck_iteration=(index_G)*this->dnn_layer->get_K() + index_K*this->current_tile->get_T_G();
    // pck_iteration = index_G*K(Number of total filters in the network)+index_K*current_tile_group
    /* END CHANGES TO SAVE MEMORY */

    // (STONNEModel.cpp->loadTile) T_R=1, T_S=T_K, T_C=1, T_K=T_N, T_G=1, T_N=1(number of input mapped ar a time), T_X_=T_M, T_Y_=1

    // if(weight distribution), else(input distribution)
    if(!this->input_finished && (pck_iteration <= current_iteration)) { //If (Flag all the inputs have been delivered && )
        //1. Weight distribution for the T_K filters
        unsigned window_size = this->current_tile->get_T_R()*this->current_tile->get_T_S()*this->current_tile->get_T_C(); // tile window size
        ...
        if(this->current_tile->get_folding_enabled()) { //If there is folding we leave a MS to perform the psum accumulation
           // T_R(1) x T_S(T_K) x T_C(1) < R(1)*S(k)*C(1). Neccesary to generate the folding signals
           // std::cout << "Aqui hay folding" << std::endl;
            window_size+=1;
            folding_shift+=1;
        }
        if(!this->weights_distributed) { // if weight not distributed -> do weight distribution (p.467 weight distribution)
            // weights not distributed for a certain iteration
            ...
            // T_N(Number of inputs mapped at a time (Only 1 is supported)) * T_X_(Number of output fmap rows) * T_Y_(Number of output fmap columns) -> multicast weight

                for(unsigned g=0; g < current_tile->get_T_G(); g++) { // Number of groups mappd at a time
                    unsigned desp_g = g*current_tile->get_T_K()*current_tile->get_T_X_()*current_tile->get_T_Y_()*window_size;
                    // desp_g = g*T_K(Number of filters and ofmap channels per group)*T_X_(Number of output fmap rows)*T_Y_(Number of output fmap columns)*window_size(tile_window)
                    for(unsigned k=0; k < current_tile->get_T_K(); k++) { // Number of filters and ofmap channels per group
                        unsigned desp_k = k*current_tile->get_T_X_()*current_tile->get_T_Y_()*window_size;
                        // desp_k = k*T_X_(Number of output fmap rows)*T_Y_(Number of output fmap columns)*window_size(tile_window)
                        for(unsigned c=0; c < current_tile->get_T_C(); c++) { // Number of input and filter channels
                            for(unsigned r=0; r < current_tile->get_T_R(); r++) { // Number of filter rows
                                for(unsigned s=0; s < current_tile->get_T_S(); s++) { // Number of filter columns
                                    //Where do I send this weight
                                    //  To every piece of N
                                    //  To every piece of T_X_
                                    //  To every piece of T_Y_
                                    bool *vector_to_send = new bool[this->num_ms]; // number of multiplier size
                                    //Itiaalizing vector
                                    for(int i=0; i<this->num_ms; i++) {
                                        vector_to_send[i]=false;
                                    }
                                 
                                    for(int n=0; n<this->current_tile->get_T_N(); n++) { // Batch size (=Number of inputs mapped at a time?)->1
                                        ...
                                        // desp_n = n*T_G(Number of groups)*T_K(Number of filters and ofmap channels per group)*T_X_(Number of output fmap rows)*T_Y_(Number of output fmap columns)*window_size(tile_window)
                                        for(int x=0; x<this->current_tile->get_T_X_(); x++) { // Number of output fmap rows
                                            for(int y=0; y<this->current_tile->get_T_Y_(); y++) { // Number of output fmap columns
                                                unsigned desp_this_neuron = x*this->current_tile->get_T_Y_()*window_size + y*window_size;
                                                // desp_this_neuron = x*T_Y_(Number of output fmap columns)*window_size(tile_window)+Y*window_size(tile_window)
                                    ...
                                    // sdmemoryStats.n_SRAM_weight_reads (Number of SRAM weight reads)
                                    ...
                                    this->sendPackageToInputFifos(pck_to_send); // weight multicast
                ...
                for(unsigned g=0; g < current_tile->get_T_G(); g++) {
                    for(unsigned k=0; k < current_tile->get_T_K(); k++) {
			            ...
			            //Creating the package with the data in memory and the destination 
                                    ...
                                    this->sendPackageToInputFifos(pck_to_send); // weight unicast
                                    //
                                }
                            }
                        }
            ...
            unsigned y_inputs = this->current_tile->get_T_S() + (this->current_tile->get_T_Y_()-1)*this->dnn_layer->get_strides(); //Number of input rows per batch of the ifmap obtained from the ofmap dimensions. 
            // i.e., filter size + number of extra Y_ Neurons mapped times the extra inputs (stride)
            // y_inputs = T_S(Number of filter columns) + T_Y_(Number of output fmap columns)-1 * get_strides
            ...
            // x_inputs = Number of filter rows + T_X_(Number of output fmap rows)-1 * get_strides
            ...
            // output_y_inputs = T_Y_(Number of output fmap columns) + get_S(Number of filter columns)-1
            unsigned output_x_inputs = this->current_tile->get_T_X_() + (this->dnn_layer->get_R()-1);
            // output_x_inputs = T_X_(Number of output fmap rows) + get_R(Number of Filter Rows)-1
            ...
            // T_Y_(Number of output fmap columns) && T_S(Number of filter columns mapped at a time) && get_strides
            // input activation is reused in MSwitches
                if(this->current_Y > 0) { //If it(current_Y : Number of input columns) is not the first column of the row, then data is reused among the Mswitches
                    if(this->current_S == (iter_S-1)) { //If it is the last iteration of S(Number of filter columns)
                        y_init = y_inputs - 1;
                        // y_inputs = T_S(Number of filter columns) + T_Y_(Number of output fmap columns)-1 * get_strides
                    }
			...
			//CHECK. There was a bug here because y_init was equals to y_inputs. If true, remove the condition
                    ...
                    y_init = 0; // first column of the row -> not reused among the Mswitches 
                }
            }
           
            for(unsigned x=0; x<x_inputs; x++) { // Number of input cols per batch of the ifmap obtained from the ofmap dimensions
                for(unsigned y=y_init; y<y_inputs; y++) { //the number of columns to iterate depends on if the fw links of the MS are enabled
                    ...
                    for(int i=0; i<this->current_tile->get_T_N(); i++) { // T_N(Batch size) 
                        destinations[i] = new bool**[this->current_tile->get_T_G()]; // T_G(Number of groups)
                            for(int g=0; g < this->current_tile->get_T_G(); g++) {
                            destinations[i][g] = new bool*[this->current_tile->get_T_C()]; // T_C(Number of input and filter channels)
                   ...
                   // std::cout << "Buscando VNs para el elemento:  [" << x << "," << y << "]" <<  std::endl;
                    ...
                    // first_possible_vn_x = x(Number of input cols per batch size) - (T_R(Number of filter rows mapped at a time) -1)
                    // = input row - filter column
                    if(first_possible_vn_x < 0) { // if x(Number of input cols per batch size) < get_T_R(Number of filter rows)-1 : can not make vn
                        first_possible_vn_x=0;
                    }
                    else { // enabled VNs
                        first_possible_vn_x = first_possible_vn_x / this->dnn_layer->get_strides();
                        // first_possible_vn_x = first_possible_vn_x / strides
                    }
                    int last_possible_vn_x = x / this->dnn_layer->get_strides();
                   // last_possible_vn_x = x(Number of input cols per batch) / strides
            ...
            //Updating variables
            this->current_S+=1;
            if(this->current_S == this->iter_S) { // currest_S == iter_S(last Number of filter columns)
            // If all the columns completed
                this->current_S = 0;
                this->current_R+=1; // current_R(current Number of flter rows), Next tile rows
                if(this->current_R == iter_R) { // current_R == iter_R(last Number of flter rows)
                    this->current_R = 0;
                    this->current_C+=1; // current_C(current Number of filter and input channels)
                    if(this->current_C == iter_C) { // current_C == iter_C(last Number of filter and input channels)
                        this->current_C = 0;
                        this->current_Y+=1; //Updating rows (Number of input columns)
                            if(this->current_Y==this->iter_Y) { // current_Y == iter_Y(last Number of input columns : last rows)
                            //Updating X
                                this->current_Y=0; //Reset Y
                                this->current_X+=1; // current_X(Number of input rows : last columns)
                                if(this->current_X == this->iter_X) { // current_X == iter_X(last Number of input rows : last columns)
                                    //If rows finished, updating next N batch
                                    this->current_X = 0; //Updating X
                                    this->current_K+=1; // current_K(Number of filters and output channels)
                                    this->weights_distributed = false; //Distribution enabled for next iteration
                                    if(this->current_K==iter_K) { // current_K == iter_K(last Number of filters and output channels)
                                        this->current_K=0;
                                        this->current_G+=1; // Updating groups count
                                        if(this->current_G==this->iter_G) { // current_G == iter_G(last Number of groups)
                                            this->current_G=0;
                                            this->current_N+=1; // Updating inputs
                                            if(this->current_N==this->iter_N) { // New inputs -> so need Weight distributions
                                            //Update K. Weight distribution neccesary first!
                                                this->current_N = 0;
                                                this->weights_distributed = true; //Avoid distribution
                                                this->weights_finished=true; // weights_finished (Flag that indicates that all the weights have been delivered)
   ...
   // }
    ...
    this->receive();
    if(!write_fifo->isEmpty()) { // write_fifo is not empty
        //Index the data by using the VN Address Table and the VN id of the packages
        for(int i=0; i<write_fifo->size(); i++) {
            DataPackage* pck_received = write_fifo->pop(); //  Data pop(from fifo) to pck_received 
            unsigned int vn = pck_received->get_vn(); //
            data_t data = pck_received->get_data(); // data = recived data from fifo -> Data in the package
            //std::cout << "Writing data: " << data << std::endl;

            assert(vn==VNAT[vn]->VN); // VNAT : registers as VN configured in the accelerator
            //std::cout << "Memory received a psum " << data << std::endl;
            unsigned int addr_offset = this->VNAT[vn]->addr;
            // addr_offset = VNAT[vn]->addr(Offset)
...
void SDMemory::sendPackageToInputFifos(DataPackage* pck) {
    ...
    //MULTICAST PACKAGE 
        ...
        // dest = Used in multicast traffic to indicate the receivers
        ...
        for(int i=0; i<this->n_read_ports; i++) { //Checking each port with size this->ms_size_per_input_port each. Total=ms_size
            ...
            unsigned int port_index = i*this->ms_size_per_input_port;  // num_ms / this->n_read_ports; -> n_read_ports = dn_bw
...
void SDMemory::send() { // 1. psum fifo (if not have any psum in fifo) -> we go to the input fifos
    //Iterating over each port and if there is data in its fifo we send it. We give priority to the psums
    for(int i=0; i<this->n_read_ports; i++) { // n_read_ports(dn_bw)
        ...
        //Fifos used to store partial psums before being fetched is not empty
            DataPackage* pck = psum_fifos[i]->pop(); //  data pop to the pck
            ...
            // sdmemoryStats.n_SRAM_read_ports_psums_use[i]++ : Number of times each read port is used to deliver a psum ++
            // count the number of read ports is used to deliver a psum

            //Sending to the connection
            this->read_connections[i]->send(pck_to_send);
            // Input port connections. There are as many connections as n_read_ports are specified.[i]->send the data
        }
        ...
        //If psums fifo is empty then input fifo is checked. If psum is not empty then else do not compute. Important this ELSE to give priority to the psums and do not send more than 1 pck
            ...
            unsigned int current_iteration=current_output_pixel / output_psums_per_channel; // calculating the iteration count
            /*std::cout << "current_output_pixel: " << current_output_pixel << std::endl;
            ...
            // k value of this package <= current_iteration
                if(pck->get_data_type()==WEIGHT) {
                    this->sdmemoryStats.n_SRAM_read_ports_weights_use[i]++; //To track information
                    // Number of times each read port is used to deliver a weight++
#ifdef DEBUG_MEM_INPUT
                    ...
                    //  Number of times each read port is used to deliver a input++
#ifdef DEBUG_MEM_INPUT
...
void SDMemory::receive() { //TODO control if there is no space in queue
    if(this->write_connection->existPendingData()) { // write_connection(Connection*)->existPendingData(Indicates if data exists)
    // if exists data
        std::vector<DataPackage*> data_received = write_connection->receive(); 
        // write_connection->receive() : if  have data return data, else return empty list
        for(int i=0; i<data_received.size(); i++) { // until output data size
            write_fifo->push(data_received[i]); // Fifo uses to store the writes before going to the memory
            // pusf the data in fifo
        }
    }

    // not existPendingData(Indicates if data exists)
    for(int i=0; i<write_port_connections.size(); i++) { //For every write port
        if(write_port_connections[i]->existPendingData()) { //  existPendingData(Data)
