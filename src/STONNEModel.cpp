...
void Stonne::connectMemoryandDSN() {
    ...
    //std::cout<< "connection Memory and DSN"<< std::endl;
...
void Stonne::loadDNNLayer(Layer_t layer_type, std::string layer_name, unsigned int R, unsigned int S, unsigned int C, unsigned int K, unsigned int G, unsigned int N, unsigned int X, unsigned int Y, unsigned int strides, address_t input_address, address_t filter_address, metadata_address_t mapping_table, address_t output_address, Dataflow dataflow) {
    ...
    this->mem->setLayer(this->dnn_layer, input_address, filter_address, mapping_table, output_address, dataflow);
}
...
void Stonne::loadGEMM(std::string layer_name, unsigned int N, unsigned int K, unsigned int M, address_t input, address_t filter, metadata_address_t mapping_table, address_t output_matrix, Dataflow dataflow) {
    ...
    //input_matrix=KN 
    //filter_matrix = MK
    loadDNNLayer(GEMM, layer_name, 1, K, 1, M, 1, 1, N, K, 1, input, filter, mapping_table, output_matrix, dataflow);
    std::cout << "Loading a GEMM + mapping_table into STONNE" << std::endl;
...
void Stonne::loadTile(unsigned int T_R, unsigned int T_S, unsigned int T_C, unsigned int T_K, unsigned int T_G, unsigned int T_N, unsigned int T_X_, unsigned int T_Y_) {
    ...
    //yujin: loadTile을 했을 때 filter가 한번에 mapping되지 않는 경우 = folding 
    ...
    this->current_tile = new Tile(T_R, T_S, T_C, T_K, T_G, T_N, T_X_, T_Y_, folding_enabled); //yujin: 연산할 때 tile단위로 연산을 수행
